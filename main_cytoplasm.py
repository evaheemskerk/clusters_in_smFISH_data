...
def main(file_path_cellpose_n, file_path_cellpose_c, file_path_outline_files, eps, min_samples, resolution_z, resolution_xy, repetition):
    data_spots = pre_process_data_set(file_name, nucleus = False, cytoplasm = True, resolution_z = 200, resolution_xy= 65)
    ...
    for i in tqdm(range(len(name_overlap_df))):
        ...
        #file_path = f"F:/Eva/outline_files/{timepoint}/FQ_outline/{outline_file}"
        #file_path_outline = f'F:/Eva/outline_files/{timepoint}/FQ_outline/{name_overlap_df.loc[i, 'ID_outline_n']}' #CY3.5?????
        ...
        merged['ID'] = merged['Cell_Name'].astype(str).apply(lambda x: f"{part_name[0]}_{part_name[1]}_{part_name[2]}_{x}")

        first_stack_nucleus = int(re.split(r'[_,\s]+', name_nucleus)[3])
        last_stack_nucleus = int(re.split(r'[_, \s]+', name_nucleus)[4])
        first_stack_cytoplasm = int(re.split(r'[_,\s]+', name_cytoplasm)[3]) #+1 # Dit moet dus weg als we het wel goed hebben gedaan!!!
        last_stack_cytoplasm = int(re.split(r'[_, \s]+', name_cytoplasm)[4])
        ...
        final_df = pd.merge(data_spots, merged, on=['ID'], how = 'inner') #this might go wrong with the suffixes, ff checken!!

        for id in final_df['ID'].unique():
            if len(final_df[final_df['ID'] == id]) >= 4:
                data = final_df[final_df.ID == id]
                # determine real number of clusters
                ...
                try:
                    ...
                    # Generate random indices for all repetitions at once
                    random_indices = np.stack([
                        rng.choice(positions_cytoplasm.shape[0], size=number_of_particles, replace=False)
                        for _ in range(repetition)
                    ])  # Shape: (repetition, number_of_particles)

                    # Extract positions all at once
                    random_positions = positions_cytoplasm[random_indices]  # Shape: (repetition, number_of_particles, 3)

                    # Scale coordinates efficiently (vectorized)
                    random_positions[:, :, 0] *= resolution_z
                    random_positions[:, :, 1:] *= resolution_xy  # Applies to both X and Y

                    for j in range(repetition):
                        N_cluster = dbscan(eps, min_samples, random_positions[j])
                        N_cluster_simulated.append(N_cluster)

                    #pvalue = (sum(n >= N_cluster_real for n in N_cluster_simulated) + 1) / (repetition + 1)
...
if __name__ == '__main__':
    ...
    final_data, error_data = main(file_path_cellpose_n, file_path_cellpose_c, file_path_outline_files, eps, min_samples, resolution_z, resolution_xy, repetition)
